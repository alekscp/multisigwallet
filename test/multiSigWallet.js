...
contract('MultiSigWallet', (accounts) => {
  ...
  describe('createTransfer', () => {
    ...
    it('Fails when called by non-approver account', async () => {
      await expectRevert(
        multiSigWallet.createTransfer(1000, recipient1, { from: nonApprover }),
  ...
  describe('sendTransfer', () => {
    it('Does not transfer if quorum is not reached', async () => {
      ...
      assert.equal(difference.toNumber(), 0);
    ...
    it('Fails when called by non-approver account', async () => {
      ...
      await expectRevert(
        multiSigWallet.sendTransfer(transferId, { from: nonApprover }),
        'Only approver allowed'
      );
    });
  });
